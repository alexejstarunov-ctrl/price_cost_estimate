...
await page.locator('.cat-item').filter({ hasText: 'Плитка на пол, прямая' }).first().locator('.cat-main').click()
...
// --- Прайс: изменить цену и имя, вернуть, скрыть, добавить, удалить, запомнить цену ---
await tab(/Расценки/); await wait(300)
const rowOf = (text) => page.locator('.cat-item').filter({ hasText: text }).first()
...
check('цена в прайсе изменена на 200', (await priceOf('Грунтовка основания')).startsWith('200₽'), await priceOf('Грунтовка основания'))
check('помечена «своя цена»', (await rowOf('Грунтовка основания').locator('.meta').innerText()).includes('своя цена'))
await page.reload({ waitUntil: 'networkidle' }); await tab(/Расценки/); await wait(300)
check('своя цена пережила перезагрузку', (await priceOf('Грунтовка основания')).startsWith('200₽'))
await rowOf('Грунтовка основания').locator('.cat-edit').click(); await page.waitForSelector('.sheet')
await page.locator('.sheet input[aria-label="Наименование"]').fill('Грунтовка (2 слоя)')
await page.locator('.sheet button[type="submit"]').click(); await wait(250)
check('имя позиции в прайсе изменено', (await rowOf('Грунтовка (2 слоя)').count()) === 1)
await rowOf('Грунтовка (2 слоя)').locator('.cat-edit').click(); await page.waitForSelector('.sheet')
await page.locator('.sheet .btn-ghost').filter({ hasText: 'Вернуть исходную' }).click(); await wait(250)
check('«Вернуть исходную» вернула имя и цену', (await priceOf('Грунтовка основания')).startsWith('130₽'), await priceOf('Грунтовка основания'))
await rowOf('Обшивка стен ГКЛ').locator('.cat-edit').click(); await page.waitForSelector('.sheet')
const hideBtn = page.locator('.sheet .btn-danger-outline')
await hideBtn.click(); await wait(100); await hideBtn.click(); await wait(250)
check('базовая позиция скрыта из прайса', (await rowOf('Обшивка стен ГКЛ').count()) === 0)
check('видна строка «Скрыто»', (await page.locator('.hidden-note').innerText()).includes('1'))
await page.locator('.hidden-note button').click(); await wait(250)
check('скрытая позиция возвращена', (await rowOf('Обшивка стен ГКЛ').count()) === 1)
await page.locator('.btn-dashed').filter({ hasText: 'В прайс' }).click(); await page.waitForSelector('.sheet')
check('заголовок «Новая позиция в прайсе»', (await page.locator('#edit-title').innerText()).includes('Новая'))
await page.locator('.sheet input[aria-label="Наименование"]').fill('Тестовая работа')
await page.locator('.sheet select[aria-label="Раздел"]').selectOption('Дополнительные работы')
await page.locator('.sheet select[aria-label="Единица"]').selectOption('шт')
await page.locator('.sheet input[aria-label="Цена"]').fill('777')
await page.locator('.sheet button[type="submit"]').click(); await wait(250)
const added = rowOf('Тестовая работа')
check('новая позиция в прайсе в нужном разделе', (await added.count()) === 1 && (await added.getAttribute('data-cat')) === 'Дополнительные работы')
check('новая позиция помечена «своя цена»', (await added.locator('.meta').innerText()).includes('своя цена'))
await added.locator('.cat-edit').click(); await page.waitForSelector('.sheet')
await page.locator('.sheet input[aria-label="Наименование"]').fill('Тестовая работа 2')
await page.locator('.sheet button[type="submit"]').click(); await wait(250)
check('своя позиция переименована', (await rowOf('Тестовая работа 2').count()) === 1)
await rowOf('Тестовая работа 2').locator('.cat-edit').click(); await page.waitForSelector('.sheet')
const delBtn2 = page.locator('.sheet .btn-danger-outline')
await delBtn2.click(); await wait(100); await delBtn2.click(); await wait(250)
check('своя позиция удалена из прайса', (await rowOf('Тестовая работа').count()) === 0)
await rowOf('Плитка на пол, прямая').locator('.cat-main').click(); await page.waitForSelector('.sheet')
await page.locator('.sheet input').nth(1).fill('1500'); await wait(100)
check('после смены цены появилась галочка «запомнить»', (await page.locator('.sheet .check').count()) === 1)
await page.locator('.sheet .check input').check()
await page.locator('.sheet input').first().fill('1')
await page.locator('.sheet button[type="submit"]').click(); await wait(300)
await tab(/Расценки/); await wait(300)
check('цена запомнилась в прайсе', (await priceOf('Плитка на пол, прямая')).startsWith('1500₽'), await priceOf('Плитка на пол, прямая'))
await rowOf('Плитка на пол, прямая').locator('.cat-edit').click(); await page.waitForSelector('.sheet')
await page.locator('.sheet .btn-ghost').filter({ hasText: 'Вернуть исходную' }).click(); await wait(250)
check('цена возвращена к базовой', (await priceOf('Плитка на пол, прямая')).startsWith('1400₽'))
